...
def insert_rows(cur,conn,schema,row):
    ...
    try:
        ...
        logger.info(f"Inserted row for video ID: {row[video_id]} into {schema}.{table}")

    except Exception as e:

        logger.error(f"Error inserting row for video ID: {row[video_id]} into {schema}.{table}: {str(e)}")
        raise e
...
def update_rows(cur,conn,schema,row):
    ...
    try:
        ...
        logger.info(f"Updated row for video ID: {row[video_id]} in {schema}.{table}")

    except Exception as e:

        logger.error(f"Error updating row for video ID: {row[video_id]} in {schema}.{table}: {str(e)}")
        raise e 
